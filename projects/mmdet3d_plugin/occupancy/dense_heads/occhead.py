...
class OccHead(nn.Module):
    def __init__(
        ...
        self.embed_dims=mid_channel
        self.sdb = SDB(channel=self.embed_dims, out_channel=self.embed_dims//2, depth=2)
        self.ssc_header = Header(out_channel, feature=self.embed_dims//2)
        ...
        if kitti360:
            kitti360_class_names =  ['empty', 'car', 'bicycle', 'motorcycle', 'truck', 'other-vehicle', 'person', 'road',
                                    'parking', 'sidewalk', 'other-ground', 'building', 'fence', 'vegetation', 'terrain',
                                    'pole', 'traffic-sign', 'other-structure', 'other-object']
            kitti360_weight = torch.from_numpy(np.array([0.464, 0.595, 0.865, 0.871, 0.717, 0.657, 0.852, 0.541, 0.602, 0.567, 0.607, 0.540, 0.636, 0.513, 0.564, 0.701, 0.774, 0.580, 0.690]))
        if self.semantic_kitti:
            self.class_names = kitti_class_names    
            if kitti360:
                self.class_names = kitti360_class_names    
            ...
            if kitti360:
                self.class_weights =  kitti360_weight
            # build train metric
    ...
    def forward_voxel(self, voxel_feats):
        ...
        for feats, occ_conv in zip(voxel_feats, self.occ_convs):
            if self.with_cp:
                ...
                feats = occ_conv(feats)
                vox_feats_diff = self.sdb(feats) # 1, C,H,W,Z
                output_occs.append(self.ssc_header(vox_feats_diff))
        # vox_feats_diff = self.sdb(voxel_feats[0]) # 1, C,H,W,Z
        # output_occs.append(self.ssc_header(vox_feats_diff))
        return output_occs
