...
class ViewTransformer(ViewTransformerLSSBEVDepth):
    def __init__(
            self, 
            loss_depth_weight,
        ...
        super(ViewTransformer, self).__init__(loss_depth_weight=loss_depth_weight, **kwargs)
    ...
    def get_downsampled_gt_depth(self, gt_depths):
        ...
        if H // self.downsample * self.downsample!= H :
            fH = H // self.downsample +1
        else:
            fH = H // self.downsample
        if W // self.downsample * self.downsample!= W :
            fW = W // self.downsample +1
        else:
            fW = W // self.downsample

        # fW = W // self.downsample #for clip
        # fH = H // self.downsample
        
        gt_depths = F.interpolate(
            gt_depths,
            (fH * self.downsample, fW * self.downsample),
            mode="nearest",
        )
        # H = (H // self.downsample+1) * self.downsample
        # W = (W // self.downsample+1) * self.downsample
