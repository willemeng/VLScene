...
class ViewTransformerLiftSplatShootVoxel(ViewTransformerLSSBEVDepth):
    def __init__(
        ...
        if imgseg:
            self.clip_fusion = nn.Sequential(nn.Conv2d(self.numC_input+512 +21, 256, 1),
                                        nn.ReLU(),
                                        nn.Conv2d(256, self.numC_input, 1))
            self.img_clip_fusion = ClipFusion(self.numC_input)
    ...
    def forward(self, input,is_train=True):
        ...
        if is_train and self.imgseg:
            clip_fuse = input[-1]
            seg_logits = input[-2]
            clip_prob = torch.softmax(seg_logits, dim=1)
            seg_mask = torch.max(seg_logits, 1)[1].unsqueeze(1)
            self.forward_dic['imgseg_logits'] = self.img_seg_head(img_feat)
            # enhanced semantic feature distillation.
            clip_fuse = self.clip_fusion(torch.cat((img_feat,clip_fuse,clip_prob),dim=1))
            clip_fuse = self.img_clip_fusion(img_feat,clip_fuse)
            kd_feat_loss = F.l1_loss(img_feat, clip_fuse, reduction='mean')

            kd_logits_loss = self.get_seg_loss(seg_mask)
            kd_loss = {'kd_feat_loss':kd_feat_loss,'kd_logits_loss':kd_logits_loss}
        ...
        return bev_feat, depth_prob , kd_loss
