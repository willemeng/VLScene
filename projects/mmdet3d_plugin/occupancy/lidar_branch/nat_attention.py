...
class NATBlock(nn.Module):
    def __init__(self, dim, depth, depth_cross, num_heads, kernel_size, downsample=True,
        ...
        self.dim = dim
        self.depth = depth
        self.adapter_img = nn.Conv2d(dim//2, dim, 1)
...
if __name__ == '__main__':
    nat_block = NATBlock(dim=128, depth=1, depth_cross=1, num_heads=8, kernel_size=7, downsample=False).cuda()

    x_in = torch.randn([1, 48, 160, 128]).cuda()
    x_multi = torch.randn([1, 48, 160, 128]).cuda()
