...
from .nat_attention import NATBlock
...
class BEVFusionv1(nn.Module):
    def __init__(self, channel):
        ...
        self.adapter_sem = nn.Conv2d(channel, channel*2, 1)
        self.adapter_com = nn.Conv2d(channel, channel*2, 1)
        self.adapter_img = nn.Conv2d(channel, channel*2, 1)

    def forward(self, bev_features, sem_features, com_features):
        sem_features = self.adapter_sem(sem_features)
        com_features = self.adapter_com(com_features)
        # img_features = self.adapter_com(img_features)

        attn_bev = self.attention_bev(bev_features)
        attn_sem = self.attention_sem(sem_features)
        attn_com = self.attention_com(com_features)
        # attn_img = self.attention_com(img_features)
...
class BEVUNetv1(nn.Module):
    def __init__(self, n_class, n_height, dilation, bilinear, group_conv, input_batch_norm, dropout, circular_padding, dropblock):
        ...
        self.img_fusion = nn.ModuleList([NATBlock(dim=channels[i]*2, depth=3, depth_cross=2, num_heads=8, kernel_size=7, downsample=False)  for i in range(3)])

    def forward(self, x, sem_fea_list, com_fea_list,image_voxel):
        x1 = self.inc(x)    # [B, 64, 256, 256]
        x2 = self.down1(x1)    # [B, 128, 128, 128]
        x2_f = self.bev_fusions[0](x2, sem_fea_list[0], com_fea_list[0]) # 128, 64, 64 -> 128
        x2_f = self.img_fusion[0](x2_f,image_voxel[0])
        # x2_f = self.bev_fusions[0](x2, sem_fea_list[0], com_fea_list[0],img_voxel_feats[0]) # 128, 64, 64 -> 128
        x3 = self.down2(x2_f)    # [B, 256, 64, 64]
        x3_f = self.bev_fusions[1](x3, sem_fea_list[1], com_fea_list[1]) # 256, 128, 128 -> 256
        x3_f = self.img_fusion[1](x3_f,image_voxel[1])

        # x3_f = self.bev_fusions[1](x3, sem_fea_list[1], com_fea_list[1],img_voxel_feats[1]) # 256, 128, 128 -> 256
        x4 = self.down3(x3_f)    # [B, 512, 32, 32]
        x4_f = self.bev_fusions[2](x4, sem_fea_list[2], com_fea_list[2]) # 512, 256, 256 -> 512
        x4_f = self.img_fusion[2](x4_f,image_voxel[2])

        # x4_f = self.bev_fusions[2](x4, sem_fea_list[2], com_fea_list[2],img_voxel_feats[2]) # 512, 256, 256 -> 512
        x5 = self.down4(x4_f)    # [B, 512, 16, 16]
