...
from projects.mmdet3d_plugin.occupancy.backbones.models.common.model.layers import *
from projects.mmdet3d_plugin.occupancy.backbones.models.common.backbones.monodepth2 import *
from projects.mmdet3d_plugin.occupancy.backbones.models.lseg.lseg_net import LSegFeatNet
from projects.mmdet3d_plugin.occupancy.backbones.models.common.model.vl_fuser import *
...
class VisionLanguageEncoder(nn.Module):
    ...
    def single_crop_forward(self, image, crop_size):
        ...
        out = resize_image(crop_image(out, 0, height, 0, crop_size), h//8, w//8, mode="bilinear")
        image_feats = resize_image(crop_image(image_feats, 0, height, 0, crop_size), h//8, w//8, mode="bilinear")
    ...
    def forward(self, x,vision_feat):
        """
        For extracting ResNet's features.
        :param x image (B, C, H, W), normalized to [-1, 1]
        :return latent (B, latent_size, H, W)
        """
        B, N, C, H, W = vision_feat.shape
        vision_feat = vision_feat.view(B*N,C,H,W)
        # vision_feat = None
        seg_mask = None
        text_feat = None
        vl_feat = None
        ...
        # fused_feat = fused_feat.view(B,N,C,H,W)
    ...
    def from_conf(cls, conf):
        ...
        label_set = get_labels('./outdoor_labels.txt')
        
        vlseg_model_name = conf.get("vlseg_model_name", "lseg")

        # vision model
        # vision_model_name = conf.get("vl_visionmodel_name", "Monodepth2")
        # vision_encoder = globals()[vision_model_name](resnet_layers=conf.get("resnet_layers",50),
        #                                 cp_location=conf.get("vision_cp_location",None),
        #                                 freeze=conf.get("freeze_vision_model",False),
        #                                 num_ch_dec=conf.get("num_ch_dec", [32,32,64,128,256]),
        #                                 d_out=conf.get("vision_d_out",64),
        #                                 scales=range(4)) if use_vision_model else None
        
        # VL model
        if vlseg_model_name == "lseg":
            vlseg_encoder = LSegFeatNet(
                                        path=conf.get("lseg_model_path",'./checkpoints/demo_e200.ckpt'),
                                        labels=label_set,
        ...
        return cls(
            ...
            freeze_vlseg_model = conf.get("freeze_vlseg_model",True),
            return_only_feat = conf.get("return_only_feat",False),
