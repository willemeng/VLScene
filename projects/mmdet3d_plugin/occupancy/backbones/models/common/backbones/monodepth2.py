...
class Decoder(nn.Module):
    ...
    def forward(self, input_features):
        with profiler.record_function("encoder_forward"):
            ...
            x = input_features[-1]
            for i in range(4, -1, -1):
                x = self.decoder[self.decoder_keys[("upconv", i, 0)]](x) # 12 40

                x = [F.interpolate(x, scale_factor=(2, 2), mode="nearest")] # 24 80
                ...
                x = self.decoder[self.decoder_keys[("upconv", i, 1)]](x) # 24 80

                self.outputs[("features", i)] = x # 24 80
