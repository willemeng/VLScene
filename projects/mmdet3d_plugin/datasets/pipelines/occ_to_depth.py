...
class CreateDepthFromLiDAR(object):
    def __init__(self, point_cloud_range, grid_size, projective_filter=True,kitti360=False,
        ...
        self.lidar_root = "data/SemanticKITTI/dataset/sequences"
        self.lidarseg_root ="data/SemanticKITTI/dataset/sequences"
        self.kitti360 = kitti360
        if kitti360:
            self.lidar_root = self.lidarseg_root = 'kitti360/'
            self.lidarseg_root ='kitti360/'
            label_mapping = './kitti360.yaml'
    ...
    def __call__(self, results):
        ...
        if self.kitti360:
            sequence, _,_, frame_id = img_filename.split("/")[-4:]

            lidar_filename = os.path.join(self.lidar_root, "data_3d_raw", sequence,"velodyne_points","data", frame_id.replace(".png" , ".bin"))
        else:
            seq_id, _, filename = img_filename.split("/")[-3:]
        ...
        if self.kitti360:
            lidarseg_filename = os.path.join(self.lidarseg_root, "data_2d_raw", sequence, 'voxels', frame_id.replace(".png", ".label"))
        else:
        ...
        # loading lidarseg labels
        if self.kitti360:
            lidarseg_filename = os.path.join(self.lidarseg_root, "data_2d_raw", 'voxels', filename.replace(".png", ".label"))
        else:
            lidarseg_filename = os.path.join(self.lidarseg_root, seq_id, "labels", filename.replace(".png", ".label"))
        # lidarseg_filename = os.path.join(self.lidarseg_root, seq_id, "labels", filename.replace(".png", ".label"))
        lidarseg = np.fromfile(lidarseg_filename, dtype=np.uint32).reshape((-1, 1))
        ...
        # results['lidar_data'] = results['lidar_data']
